...
def find_matched_running_test_instance(imageId, type):
...
def deploy(ami,keyname,user,userdata,type,securitygroup, reuse=True):
    
    print("Launching: %s on %s" % (ami,type))
    
    if (reuse):
        print "Trying to reuse existing instances"
        targetInstances = find_matched_running_test_instance(ami, type)
    ...
    if (not targetInstances):
        ...
        targetInstances = res.instances;
        
    ensure_running(instances=targetInstances)
        
    hosts = [user + "@" + instance.public_dns_name for instance in targetInstances]
    launched = [{'host':instance.public_dns_name,
                 'host_string':user + "@" + instance.public_dns_name,
                 'id':instance.id} for instance in targetInstances]
...
def deploy_node(node,running):        
        ...
        for i in range(0,node['instances']):
            ...
            print "Using private key: %s" % node['ssh_key']
            print "Host string: %s" %launched[0]['host_string']
